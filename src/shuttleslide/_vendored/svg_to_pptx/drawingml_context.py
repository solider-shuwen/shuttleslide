...
class ConvertContext:
    ...
    def child(
        ...
        return ConvertContext(
            ...
            # Compose parent scale into the translate delta: a translate(dx)
            # in the child's SVG-local space shifts the parent's already-
            # scaled coordinate system by scale*dx in slide-px space, not
            # dx. Without the scale factor, nested <g transform="translate(..)">
            # drifts by dx*(1 - scale_x) per level — for a 1280px SVG
            # rendered at 1000px (scale 0.78) with translate(213,260),
            # the icon lands at slide (274, 375)px instead of (228, 319)px,
            # a +46px / +57px error that compounds deeper. This matches
            # the matrix-composition branch above (e1*a2 + c1*b2 + ...)
